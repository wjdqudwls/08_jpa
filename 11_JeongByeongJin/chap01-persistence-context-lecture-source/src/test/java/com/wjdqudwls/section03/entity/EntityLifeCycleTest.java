package com.wjdqudwls.section03.entity;
...
class EntityLifeCycleTest {
  ...
  void testTransient(int menuCode) {
    ...
    // foundMenu : EntityManager를 통해 DB에서 조회한 객체 -> 영속상태
  ...
  void testManagedOtherEntityManager(int menuCode) {
    ...
    // 새로운 엔티티가 생성됨 -> 서로 독립된 연속성 컨텍트를 관리
  ...
  void testDetachEntity(int menuCode, int menuPrice) {
    ...
    //    (더티 채킹 : @Entity 객체의 변화(DB와 스냅샷 차이)를 감지)
    // (현재)준영속 상태
