package com.wjdqudwls.association.section02.onetomany;
...
public class Category {
  ...
  /* @OneToMany
   * - 일대다 관계 관계를 매핑할 때 사용
   * - Category 엔티티 하나가 여러 Menu 엔티티(N)를 가질 수 있는 관계
   *
   * [주의점]
   * 1. 일대다 단방향 매핑은
   *    햔재 엔티티(Category)가 아니라 반대편 엔티티(Menu)에 외래키(FK)가 있기 떄문에
   *    연관 관계 처리를 위해서 UPDATE SQL이 추가적으로 실행될 수 있다.
   *    -> 현재 엔티티의 PK와 반대편 엔티티의 FK를 직접 등록 해줘도
   *      불필요한 UPDATE SQL이 수행된다.
   *
   * 2. FetchType.LAZY 시 "N+1 문제"가 발생할 수 있다.
   *
